...
\[O(g(n))=\{f(n):(\exists c>0, n_0 \geq 0)(\forall n \geq n_0)(0 \leq f(n) \leq c g(n))\}\]
Nagy ordó egy halmaz amelyekben adott függvények vannak, jelöljünk 
egy ilyen függvényt  $f(n)$-el. 
$f(n)$ függvény ahol létezik egy $c$ konstans úgy, hogy lesz egy $n_0$
...
\includegraphics[height=3cm]{xpoli}\\$$\text{Polinomiális - }O(\sum_{i=0}^{k}a_in^i)(a_k>0)$$\\
